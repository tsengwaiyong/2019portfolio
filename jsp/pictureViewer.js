$(document).scroll(function() {
     $('.titlebar img').css('top', $(document).scrollTop() / 2);
})
function totop(){
...
$(function () {
	...
	$('.tab1').on('click', '.cover', function () {
		var this_ = $(this);
		var images = this_.parents('.work').find('.cover');
	...
	$('.tab2').on('click', '.cover', function () {
		var tab1lenght = $(".tab1 .cover").length + 1;
		var this_ = $(this);
		var images = this_.parents('.work').find('.cover');
		var imagesArr = new Array();
		$.each(images, function (i, image) {
		...
		$.pictureViewer({
			images: imagesArr,
			initImageIndex: this_.index() + tab1lenght,
			scrollSwitch: true
		});
	});
...
(function ($, document, window) {
    $.pictureViewer = function (options) {
		...
		var $bgclosebtn = $pictureViewer.find('.bg-close-btn');
